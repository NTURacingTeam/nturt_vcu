  ...
  vehicle_control_ExtU *rtU, vehicle_control_ExtY *rtY)
{
...

  {
    ...
    /* MATLAB Function: '<S15>/MATLAB Function4' */
    ...
    /* MATLAB Function: '<S25>/reference yaw rate 1' incorporates:
     *  Gain: '<S24>/Gain4'
     *  Lookup_n-D: '<S24>/SteerWheel to SteerAngle(L)'
     *  Lookup_n-D: '<S24>/SteerWheel to SteerAngle(R)'
     *  SignalConversion generated from: '<Root>/Bus Selector1'
     *  Sum: '<S24>/Add'
     */
    rtb_F_aero = (look1_binlx(rtb_torqueCmd, rtConstP.pooled8,
      rtConstP.SteerWheeltoSteerAngleL_tableDa, 100U) + look1_binlx
                  (rtb_torqueCmd, rtConstP.pooled8,
                   rtConstP.SteerWheeltoSteerAngleR_tableDa, 100U)) * 0.5 *
      rtU->vehicle_state.velocity.x / (rtU->vehicle_state.velocity.x *
      rtU->vehicle_state.velocity.x * -0.00029411764705882274 + 1.6);
     ...
     *  Sum: '<S4>/Subtract'
     */
    if ((rtU->imu.gyro.z - rtb_F_aero) * rtb_Sum > 0.0) {
    ...
    rtb_Gain_k = ((0.672 * rtb_F_dy - 0.92799999999999994 * rtb_uDLookupTable) /
                  ...
                  (rtU->vehicle_state.velocity.x * rtU->vehicle_state.velocity.x)
                  / 1.6 * 0.0 * (1.0 - fb_ratio) + (rtb_F_aero - rtU->imu.gyro.z)
                  / (5.0 * rtb_Gain_k) * fb_ratio) / 1.24 * 0.259 * tv_gain;
    ...
    rtb_F_dy = 0.10471975511965977 * rtb_Gain;

    /* Gain: '<Root>/Gain1' */
    rtb_uDLookupTable = 13.1 * rtU->wheel.speed.rr;

    /* UnitConversion: '<S2>/Unit Conversion' */
    /* Unit Conversion - from: rpm to: rad/s
       Expression: output = (0.10472*input) + (0) */
    rtb_FzFR = 0.10471975511965977 * rtb_uDLookupTable;
    ...
    if (rtU->vehicle_state.velocity.x < 1.0) {
      ...
      rtb_Subtract = 0.91432160643834726 * rtU->imu.gyro.z * 0.67809837986346067
        + rtU->vehicle_state.velocity.x;
      Vx_l = rtU->vehicle_state.velocity.x - 0.91432160643834726 *
        rtU->imu.gyro.z * 0.67809837986346067;
      rtDW->SR_r = (0.259 * rtb_F_dy / 13.1 - rtb_Subtract) / rtb_Subtract;
      rtDW->SR_l = (0.259 * rtb_FzFR / 13.1 - Vx_l) / Vx_l;
    }

    /* End of MATLAB Function: '<S4>/MATLAB Function4' */

    /* Abs: '<S22>/Abs' */
    rtb_torqueCmd_b = fabs(rtDW->SR_l);

    /* Derivative: '<S22>/Derivative' incorporates:
     *  Derivative: '<S23>/Derivative'
     */
    Vx_l = rtM->Timing.t[0];
    if ((rtDW->TimeStampA >= Vx_l) && (rtDW->TimeStampB >= Vx_l)) {
      ...
      if (rtDW->TimeStampA < rtDW->TimeStampB) {
        if (rtDW->TimeStampB < Vx_l) {
          rtb_torqueCmd = rtDW->TimeStampB;
          lastU = &rtDW->LastUAtTimeB;
        }
      } else if (rtDW->TimeStampA >= Vx_l) {
        rtb_torqueCmd = rtDW->TimeStampB;
        lastU = &rtDW->LastUAtTimeB;
      }

      /* Derivative: '<S22>/Derivative' */
      rtb_torqueCmd = (rtDW->SR_l - *lastU) / (Vx_l - rtb_torqueCmd);
    }
    ...
    if (rtU->vehicle_state.velocity.x < 1.0) {
      ...
      rtb_Subtract = atan((rtU->vehicle_state.velocity.y - rtU->imu.gyro.z *
    ...
    MATLABFunction_p(rtU->cockpit.brake, rtDW->Memory, rtDW->Memory1, fabs
                     (rtb_Sum), rtDW->target_SR - rtb_torqueCmd_b, rtb_torqueCmd,
                     rtb_Subtract, &rtb_torqueCmd_b, &rtDW->tc_on_l);

    /* Saturate: '<S22>/Saturation' */
    if (rtb_torqueCmd_b > 1000.0) {
      /* Saturate: '<S22>/Saturation' */
      rtDW->Saturation = 1000.0;
    } else if (rtb_torqueCmd_b < 0.0) {
      /* Saturate: '<S22>/Saturation' */
      rtDW->Saturation = 0.0;
    } else {
      /* Saturate: '<S22>/Saturation' */
      rtDW->Saturation = rtb_torqueCmd_b;
    }
     ...
     *  Gain: '<S4>/Gain8'
     */
    rtb_torqueCmd_b = look1_binlx(9.5492965855137211 * rtb_FzFR,
      rtConstP.pooled13, rtConstP.pooled12, 14U);

    /* Gain: '<S4>/Gain11' */
    rtb_FzFR = 0.39999999999999997 * rtb_torqueCmd_b;
     ...
     *  Signum: '<S22>/Sign'
     */
    MATLABFunction1(rtb_Sum * rtDW->Saturation, rtb_FzFR, &rtb_torqueCmd_b);

    /* MATLAB Function: '<S3>/MATLAB Function1' */
    MATLABFunction(rtb_torqueCmd_b, rtb_Gain, &rtb_torqueCmd);
    if (tmp) {
    ...
    if (rtb_torqueCmd > rtDW->Constant1) {
      rtb_torqueCmd = rtDW->Constant1;
    } else if (rtb_torqueCmd < -torq_limit_rl) {
      /* Switch: '<S7>/Switch' incorporates:
       *  Constant: '<S3>/Constant'
       */
      rtb_torqueCmd = -torq_limit_rl;
    }
    ...
    if (rtDW->LastMajorTime == (rtInf)) {
      /* RateLimiter: '<S3>/Rate Limiter1' */
      rtDW->RateLimiter1 = rtb_torqueCmd;
    } else {
      rtb_Sum = rtM->Timing.t[0];
      rtb_Gain = rtb_Sum - rtDW->LastMajorTime;
      if (rtDW->LastMajorTime == rtb_Sum) {
        if (rtDW->PrevLimited) {
          /* RateLimiter: '<S3>/Rate Limiter1' */
        ...
        rtb_torqueCmd_b = rtb_Gain * torq_slew;
        rtb_Sum = rtb_torqueCmd - rtDW->PrevY;
        if (rtb_Sum > rtb_torqueCmd_b) {
          /* RateLimiter: '<S3>/Rate Limiter1' */
          rtDW->RateLimiter1 = rtDW->PrevY + rtb_torqueCmd_b;
          limitedCache = true;
        } else {
          rtb_Gain *= -torq_slew;
          if (rtb_Sum < rtb_Gain) {
    ...
    rtb_torqueCmd_b = 66.666666666666671 * rtX->TransferFcn1_CSTATE;

    /* Gain: '<Root>/Gain2' incorporates:
     *  Sum: '<S4>/Sum1'
     */
    rtb_Gain_k += rtb_torqueCmd_b;
    ...
    if (tmp) {
      ...
      /* Constant: '<S23>/target_SR' */
      rtDW->target_SR_n = target_sr;
    }

    /* Abs: '<S23>/Abs1' */
    rtb_Sum = fabs(rtb_Gain_k);

    /* Gain: '<Root>/Gain2' incorporates:
     *  Abs: '<S23>/Abs'
     */
    rtb_Gain_k = fabs(rtDW->SR_r);

    /* Derivative: '<S23>/Derivative' */
    if ((rtDW->TimeStampA_g >= Vx_l) && (rtDW->TimeStampB_i >= Vx_l)) {
      /* Derivative: '<S23>/Derivative' */
      Vx_l = 0.0;
    } else {
      ...
      if (rtDW->TimeStampA_g < rtDW->TimeStampB_i) {
        if (rtDW->TimeStampB_i < Vx_l) {
          rtb_torqueCmd = rtDW->TimeStampB_i;
          lastU = &rtDW->LastUAtTimeB_e;
        }
      } else if (rtDW->TimeStampA_g >= Vx_l) {
        rtb_torqueCmd = rtDW->TimeStampB_i;
        lastU = &rtDW->LastUAtTimeB_e;
      }

      /* Derivative: '<S23>/Derivative' */
      Vx_l = (rtDW->SR_r - *lastU) / (Vx_l - rtb_torqueCmd);
    }
    ...
    MATLABFunction_p(rtU->cockpit.brake, rtDW->Memory_j, rtDW->Memory1_p,
                     rtb_Sum, rtDW->target_SR_n - rtb_Gain_k, Vx_l, rtb_Subtract,
                     &rtb_Gain_k, &rtDW->tc_on);
     ...
     */
    rtb_Gain_k = look1_binlx(9.5492965855137211 * rtb_F_dy, rtConstP.pooled13,
      rtConstP.pooled12, 14U);

    /* Gain: '<S4>/Gain12' */
    rtb_F_dy = 0.39999999999999997 * rtb_Gain_k;

    /* MATLAB Function: '<S23>/MATLAB Function1' incorporates:
     *  Product: '<S23>/Product'
     */
    MATLABFunction1(rtb_Gain * rtDW->Saturation_d, rtb_F_dy, &rtb_Gain_k);

    /* MATLAB Function: '<S3>/MATLAB Function' */
    MATLABFunction(rtb_Gain_k, rtb_uDLookupTable, &rtb_torqueCmd_b);
     ...
     *  Switch: '<S8>/Switch'
     */
    if (rtb_torqueCmd_b > rtDW->Constant2) {
      rtb_Gain_k = rtDW->Constant2;
    } else if (rtb_torqueCmd_b < -torq_limit_rr) {
    ...
    if (rtDW->LastMajorTime_l == (rtInf)) {
      ...
      rtb_Sum = rtM->Timing.t[0];
      rtb_Gain = rtb_Sum - rtDW->LastMajorTime_l;
      if (rtDW->LastMajorTime_l == rtb_Sum) {
        ...
        rtb_torqueCmd_b = rtb_Gain * torq_slew;
        rtb_Sum = rtb_Gain_k - rtDW->PrevY_o;
        if (rtb_Sum > rtb_torqueCmd_b) {
          /* RateLimiter: '<S3>/Rate Limiter' */
          rtDW->RateLimiter = rtDW->PrevY_o + rtb_torqueCmd_b;
          limitedCache = true;
        } else {
          rtb_Gain *= -torq_slew;
          if (rtb_Sum < rtb_Gain) {
    ...
    /* Product: '<S4>/Product1' */
    rtDW->Product1 = rtb_Gain_k * rtb_F_dy;

    /* Product: '<S4>/Product' */
    rtDW->Product = rtb_FzFR * rtb_Gain_k;
    if (tmp) {
      ...
      /* MATLAB Function: '<S21>/MATLAB Function5' incorporates:
       *  Memory: '<S21>/Memory'
       */
    ...
    rtY->tc.sr_r = rtDW->SR_r;
    rtY->tc.sa = rtb_Subtract;
