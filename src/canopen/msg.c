...
CANOPEN_OD_TO_MSG_DEFINE(msg_sensor_cockpit,
    ...
    OD_TO_MSG_ENTRY(0x2080,
        OD_TO_MSG_DATA(0x0, int16_t, IDENTITY, AGG_MEMBER(steer))
    ),
    OD_TO_MSG_ENTRY(0x2081,
        OD_TO_MSG_DATA(0x1, uint8_t, IDENTITY, AGG_MEMBER(accel_pedal_plaus))
    ),
    OD_TO_MSG_ENTRY(0x2082,
        OD_TO_MSG_DATA(0x1, uint8_t, IDENTITY, AGG_MEMBER(brake))
    )
);
